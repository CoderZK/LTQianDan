...
- (void)config{
    ...
    _scrollView.contentSize = CGSizeMake(ScreenW*3, height);
    //不显示水平进度条
    _scrollView.showsHorizontalScrollIndicator = NO;
    _scrollView.pagingEnabled = YES;
    for (int i=0; i < 3; i++) {
        UIImageView *imgView = [[UIImageView alloc] init];
        imgView.image = [UIImage imageNamed:[NSString stringWithFormat:@"guide%d",i+1]];
        ...
        if (i == 2) {
            ...
            comeinBtn.frame = CGRectMake(0, 0, ScreenW, ScreenH);
//            [comeinBtn setBackgroundImage:[UIImage imageNamed:@"letwogo"] forState:UIControlStateNormal];
            [comeinBtn setTitle:@"" forState:0];
            comeinBtn.layer.cornerRadius = 5;
            comeinBtn.layer.borderColor = [UIColor clearColor].CGColor;
